...
def main
  ...
  if (ARGV.size == 1) && (ARGV[0] == 'revert')
    ...
    state['attachments'].each do |attachment|
      ...
      puts 'skipping creation of attachment due to error: ' + e.to_s
    end
    state['grants'].each do |attachment|
      ...
      puts 'skipping creation of grant due to error: ' + e.to_s
    end
  ...
  users.each do |user|
    ...
    begin
      ...
      puts 'skipping user ' + user.id + ' on account of error: ' + e.to_s
    end
  end
